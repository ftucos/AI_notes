...
This means that for a matrix $\boldsymbol{A} \in \mathbb{R}^{n \times n}$ if $\det(\boldsymbol{A}) =0$ then $\mathrm{rank}(\boldsymbol{A})<n$
...
$\mathrm{tr}(\boldsymbol{A})= \sum_{i=1}^{n} a_{ii}= a_{11} + a_{22} + \dots + a_{nn}$
Useful properties:
